...
**DELETE THIS FILE WHEN `0013_stage02_handshake` LANDS ON MAIN.**

PRD §7.2 lists `plan_calc` alongside `research_acceptance` and `campaign_plan`,
and §23 assigns all three to S2-P0's `stage02_handshake` revision — which
already creates it, on `feat/s2-p0-handshake`. §21 also specifies S2-P1 as
buildable in parallel with the handshake, and `calc/derived.py` is this table's
only writer: its dedupe behaviour is one of S2-P1's three exit criteria and
cannot be proven against a table that does not exist. So both branches carry
the table, and one copy has to go at merge time.

Two branches cannot both be independently migratable *and* avoid claiming
revision 0013 — whichever merges second leaves two alembic heads, and
`alembic upgrade head` then refuses to run. The mitigation is that the table is
defined here **identically** to S2-P0's: same columns, same nullability, same
`uq_plan_calc_run_formula_inputs` and `ix_plan_calc_run_node` names. Reconciling
is therefore "delete this file and the `PlanCalc` class in `db/models.py`", with
no change to anything in `agent/calc/`.

`plan_calc` references only `run` and `evidence`, both of which predate Stage
02, so it carries no dependency on the handshake's own tables.
