...
WEB_PORT="${WEB_PORT:-3421}"
export API_INTERNAL_URL="http://127.0.0.1:8421"
export WORKER_INTERNAL_URL="http://127.0.0.1:8423"
export NEXT_TELEMETRY_DISABLED=1
export SHOTS="${SHOTS:-/tmp/s4p21-shots}"
compose=(docker compose -p s4p21 -f "$REPO/docker-compose.yml" -f "$REPO/apps/web/scripts/s4p21/compose.s4p21.yml")
...
for _ in $(seq 1 60); do
  curl -fsS "http://127.0.0.1:8421/api/v1/health" >/dev/null 2>&1 && break
  sleep 1
done
...
pnpm exec next start -p "$WEB_PORT" -H 127.0.0.1 >/tmp/s4p21-web.log 2>&1 &
web=$!
