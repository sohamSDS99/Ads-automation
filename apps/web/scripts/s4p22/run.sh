...
# `s4p24-22`): S4_PROJECT, S4_API_PORT, S4_FILES_PORT (file server),
...
# (`down -v`) on exit.
export S4_PROJECT="${S4_PROJECT:-s4p22}"
export S4_API_PORT="${S4_API_PORT:-8522}"
export S4_FILES_PORT="${S4_FILES_PORT:-8523}"
export S4_AUX_PORT="${S4_AUX_PORT:-8524}"
export S4_SUBNET="${S4_SUBNET:-172.31.222.0/24}"
export S4_SUBNET6="${S4_SUBNET6:-fd00:ada:222::/64}"
export WEB_PORT="${WEB_PORT:-3522}"
...
compose=(docker compose -p "$S4_PROJECT" -f "$REPO/docker-compose.yml" -f "$REPO/apps/web/scripts/s4p22/compose.s4p22.yml")
web=""
cleanup() {
