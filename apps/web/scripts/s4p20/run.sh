...
# `s4p24-20`): S4_PROJECT, S4_API_PORT, S4_AUX_PORT (openrouter), S4_SUBNET,
...
export S4_PROJECT="${S4_PROJECT:-s4p20}"
export S4_API_PORT="${S4_API_PORT:-8502}"
export S4_AUX_PORT="${S4_AUX_PORT:-8503}"
export S4_SUBNET="${S4_SUBNET:-172.31.202.0/24}"
export S4_SUBNET6="${S4_SUBNET6:-fd00:ada:202::/64}"
export WEB_PORT="${WEB_PORT:-3502}"
...
compose=(docker compose -p "$S4_PROJECT" -f "$REPO/docker-compose.yml" -f "$REPO/apps/web/scripts/s4p20/compose.s4p20.yml")
web=""
cleanup() {
