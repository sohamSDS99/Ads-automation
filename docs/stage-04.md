...
|---|---|---|
| CC1 wall clock | **NOT MET: copy track** | Zero-latency machine time scaled to the CC1 shape: 27.8–55.7 s against a 270 s budget; copy track 20–40 s against 72 s. But the executor runs waves, and 4.3.3's wave comes after waves holding 4.4.4 (video) and 4.4.5 (G8, a human stop). So the copy track waits for video generation (7/7 runs; 21.3–33.8 s with 8 s of video latency; a video may take up to 900 s, over the 12-minute copy budget) *and* for the G8 decision. A ratchet pins that shape so it cannot grow (`tests/creative/test_cc1_copy_track_structure.py`). The live threshold is not measurable offline. `test_s4p24_cost_and_time.py` |
| CC2 cost | text **met**; caps **NOT MET** | Text at default routing: $0.20–$0.26 per golden run, $1.14–$2.03 scaled to 10 ad groups (≤ $6). The caps can be passed by actual − estimate: $40.1125 vs $40 and $50.1125 vs $50; $85.00 vs $40 at the recorded wan billing with 400 jobs. It never compounds, and ratchets pin each overshoot at its measured value. `test_s4p24_cc2_media_caps.py`, `test_media_budget.py` |
